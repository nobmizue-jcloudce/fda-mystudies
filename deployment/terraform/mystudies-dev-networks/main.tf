...
resource "google_compute_firewall" "fw_allow_k8s_ingress_lb_health_checks" {
  ...
  target_tags = [
    "gke-mystudies-dev-gke-cluster",
    "gke-mystudies-dev-gke-cluster-default-node-pool",
  ]
}
...
module "bastion_vm" {
  ...
  network      = module.mystudies_dev_network.network.network.self_link
  subnet       = module.mystudies_dev_network.subnets["asia-northeast1/mystudies-dev-bastion-subnet"].self_link
  members      = ["group:mystudies-dev-bastion-accessors@jcloudce.com"]
  ...
  nats = [
    {
      ...
      subnetworks = [
        {
          name                     = "${module.mystudies_dev_network.subnets["asia-northeast1/mystudies-dev-bastion-subnet"].self_link}"
          source_ip_ranges_to_nat  = ["PRIMARY_IP_RANGE"]
          secondary_ip_range_names = []
        },
        {
          name                     = "${module.mystudies_dev_network.subnets["asia-northeast1/mystudies-dev-gke-subnet"].self_link}"
